# Per-note tag suggestion task (closed taxonomy v2, round-3)

You are re-tagging short personal notes against a **fixed 32-tag canonical
...
This is a **round-3 incremental pass** over notes ingested into prod AFTER the
round-2 bulk run. The taxonomy was promoted at the end of round-2 — `people`,
`transportation`, and `watch-list` are now canonical. Use them.
...
(`cluster_id` is omitted in round-3 — no clustering pass was needed for the
incremental batch. Tag from the text alone.)
...
- `suggested_tags`: 1–3 tags, **strictly drawn from the 32-tag canonical list below**. Lowercase, kebab-case verbatim. **Never** put an out-of-vocab tag here.
- `primary`: one of the `suggested_tags` (drives note background color).
- `proposed_tag`: `null` by default. If — and only if — you have **high confidence** that a missing tag would fit AND none of the 32 canonical tags do, emit a single kebab-case lowercase string here. One word or hyphenated. This is reviewed offline; it does NOT enter `suggested_tags`. Expect this to be rare in round-3 — the taxonomy is broader now.
...
## Canonical 32-tag taxonomy (CLOSED SET — `suggested_tags` MUST be a subset)
...
writing, finance, to-learn, people, transportation, watch-list
```
...
| `people` | Specific person, contact, name, relationship reference. Subsumes `contact`. |
| `transportation` | Cars, bikes, planes, trains, travel logistics. |
| `watch-list` | Movies, shows, videos queued to watch. Distinct from `reading-list`. |
...
"call mom thursday"               → {"suggested_tags":["todo","people"],"primary":"todo","proposed_tag":null,"confidence":"high","rationale":"action item involving person"}
"tesla model y 2026 review"       → {"suggested_tags":["transportation","reading-list"],"primary":"transportation","proposed_tag":null,"confidence":"high","rationale":"car review reference"}
"severance s2 ep5"                → {"suggested_tags":["watch-list"],"primary":"watch-list","proposed_tag":null,"confidence":"high","rationale":"specific show episode"}
```

## Edge cases

- Empty/whitespace → `{"suggested_tags":["unclassifiable"],"primary":"unclassifiable","proposed_tag":null,"confidence":"low","rationale":"empty"}`
- Pure URL → `["link"]` (+ `tweet` / `article` / `watch-list` if domain implies — youtube.com is `watch-list`, x.com is `tweet`).
- Mixed-topic notes → pick the **dominant** theme for `primary`.
- A plausible missing tag (e.g. `education`, `engineering`) → set `proposed_tag` to it, keep `suggested_tags` in-vocab (fall back to nearest in-vocab tag, or `unclassifiable` if none). Note: `people`, `transportation`, `watch-list` are NOW canonical — do not propose them.
...
- Every entry in `suggested_tags` MUST be one of the 32 canonical tags.
- `primary` MUST be present in `suggested_tags`.
- `proposed_tag` MUST be `null` or a single lowercase kebab-case string outside the 32 canonical tags.
...
- Don't propose `people`, `transportation`, or `watch-list` — they are already canonical.
